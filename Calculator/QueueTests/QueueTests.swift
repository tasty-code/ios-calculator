...
final class QueueTests: XCTestCase {
    ...
    func test_enqueue로_item이_들어갔는지_확인() {
        let input: Item = Item(value: 13.0, operator: .addition)
        sut.enqueue(input)
    ...
    func test_dequeue가_제대로_되었는지_확인() {
        let input: Item = Item(value: 123, operator: .addition)
        let input1: Item = Item(value: 456, operator: .addition)
        let input2: Item = Item(value: 789, operator: .addition)
        sut.enqueue(input)
        sut.enqueue(input1)
        sut.enqueue(input2)
        sut.dequeue()
